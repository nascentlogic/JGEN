...
public class BipedLocomotion {
    ...
    public BipedLocomotion(float bodyRadius) {
        this(
                bodyRadius,
                DEFAULT_TORSO_F,
                DEFAULT_TORSO_ZETA,
    ...
    public void update(Vector2f bodyPos, Vector2f velocity, Vector2f acceleration, Vector2f viewDir, float dt) {
        // Safe linear interpolation factor clamped to protect against extreme frame-rate drops
        float accelAlpha = Math.min(dt * ACCEL_SMOOTH_RATE, 1.0f);
        smoothedAcceleration.lerp(acceleration, accelAlpha);
        ...
        float maxLean = bodyRadius * 0.35f; // todo: hardcoded global variable
        if (targetLean.lengthSquared() > maxLean * maxLean) {
            targetLean.normalize(maxLean);
        } // Apply visual-only offset interpolation with safe alpha ceiling limits
        float leanAlpha = Math.min(dt * TORSO_LEAN_SMOOTH_RATE, 1.0f);
        torsoOffset.lerp(targetLean, leanAlpha);
    ...
    public static final class FeetTracker {
        ...
        void update(Vector2f bodyPos, Vector2f velocity, Vector2f smoothedAccel, float bodyRadius, MovementState state, float torsoAngle, float dt) {
            // Progress active step positions along their flat plane trajectories
        ...
        private void applyHardConstraint(Foot foot, Vector2f homePos, float bodyRadius) {
            ...
            float distSq = toFoot.lengthSquared();
            float maxLag = bodyRadius * MAX_FOOT_LAG_FACTOR;
            if (distSq > maxLag * maxLag) {
                float dist = Math.sqrt(distSq);
                toFoot.div(dist).mul(maxLag);
